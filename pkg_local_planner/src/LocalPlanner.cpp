...
void LocalPlanner::onRecPath(const nav_msgs::Path::ConstPtr& msg){
	...
	for(int i=0;i<pCount-1;i++){
		double x = msg->poses[pCount-i-1].pose.position.x;
		double y = msg->poses[pCount-i-1].pose.position.y;
		double th = tf::getYaw(msg->poses[pCount-i-1].pose.orientation);
		Pose* tmp = new Pose(x,y,th);
		path.push_back(*tmp);
	}
	Pose* tmp = new Pose(0, 0, 0);
	path.push_back(*tmp);
}
...
//printState("in thr",0,0);                                 
				...
				pathIdx++;
				 ...
				 //解决到达送餐点后避障的问题
				double biasAng0;
				biasAng0 = getBiasAng(robotPose.th, getAng(robotPose, path[pathIdx]));
				int spinDir0 = biasAng0 / fabs(biasAng0);
...
//printState("arrive the last second goal,waiting for taking meals",0,0);
				...
				//解决回原点后朝向问题
				double biasAng;
				biasAng = getBiasAng(robotPose.th, 0);
				int spinDir = biasAng / fabs(biasAng);
				if (fabs(biasAng) > angLimit) {
					while (ros::ok() && fabs(biasAng) > angThreshold) {
						ros::spinOnce();
